...
function SummaryCard({ label, value, sub, accent }) {
  return (
    <div className="cost-card" style={{ '--card-accent': accent }}>
      ...
      <div className="cost-card__value">{fmt(value)}</div>
    ...
    <div className="cost-table-wrap">
      {dayOrder.map(day => (
        <div key={String(day)} className="cost-day-group">
          <div className="cost-day-label">{DAY_LABEL[day] ?? `Ngày ${day}`}</div>
          <table className="cost-table">
            ...
            <tbody>
              {byDay[day].map((r, i) => (
                <tr key={i}>
                  <td>
                    <span className="cost-cat-icon">{CAT_ICON[r.category] ?? '📌'}</span>
                    {r.item}
                    {r.note ? <span className="cost-note"> · {r.note}</span> : null}
                  </td>
                  <td className="cost-td-num">{fmt(r.unitPrice)}</td>
                  <td className="cost-td-num cost-td-qty">{r.qty} {r.unit}</td>
            ...
            <tfoot>
              <tr className="cost-subtotal">
                ...
                <td className="cost-td-num">{fmt(byDay[day].reduce((s, r) => s + r.total, 0))}</td>
              </tr>
            </tfoot>
          </table>
        </div>
  ...
  if (!rows.length) {
    return (
      <div className="cost-empty">
        ...
        <div className="cost-empty__text">Chưa có chi phí thực tế nào được ghi lại.</div>
        <div className="cost-empty__sub">Vào Google Sheet → tab "Thực tế" để điền khi đã chi.</div>
      </div>
    )
  }
          ...
          <table className="cost-table">
            ...
            <tfoot>
              <tr className="cost-subtotal">
                ...
                <td className="cost-td-num">{fmt(byDay[day].reduce((s, r) => s + r.amount, 0))}</td>
              </tr>
            </tfoot>
          </table>
        </div>
...
export default function CostView() {
  ...
  const { budget, actual, budgetTotal, actualTotal, loading, error } = useSheetCosts()
  ...
  return (
    <div className="cost-view">
      ...
      <div className="cost-view-body">
        {loading && <div className="cost-loading">Đang tải dữ liệu chi phí…</div>}
        {error   && <div className="cost-error">⚠ {error}</div>}
        {!loading && !error && tab === 'budget' && <BudgetTable rows={budget} />}
        {!loading && !error && tab === 'actual' && <ActualTable rows={actual} budgetTotal={budgetTotal} />}
      </div>
    </div>
  )
}
